...
{
    rk_f1(density_grid_aux1, k1_grid, dt/2);
    rk_f2f3(density_grid_aux1, density_grid_aux2, k2_grid, dt/2);
    rk_f2f3(density_grid_aux2, density_grid_aux1, k3_grid, dt);
    rk_f4_stochastic(density_grid_aux1, k1_grid, k2_grid, k3_grid, rng, dt/6);
}
...
)
{
    for (auto i=0; i<n_cells; i++)
    {
        ...
        density_grid_aux[i] = density_grid[i] + dt_fraction*k1_grid[i];
    }
}
...
)
{
    for (auto i=0; i<n_cells; i++)
    {
        k23_grid[i] = nonlinear_rhs(i, density_grid_aux_in);
        density_grid_aux_out[i] = density_grid[i] + dt_fraction*k23_grid[i];
    }
}
